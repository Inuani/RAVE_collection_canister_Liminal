...
module MaxBpTree {
    ...
    public class MaxBpTree<K, V>(cmp_key : CmpFn<K>, cmp_val : CmpFn<V>, opt_order : ?Nat) {
        let order = Option.get(opt_order, 32);
        ...
        var root = #leaf(
            Leaf.new<K, V>(order, 0, null, gen_id, func(_ : V, _ : V) : Int8 = 0)
        );
    ...
    public func _replace_at_leaf_index(max_bp_tree : MaxBpTree<Nat, Nat>, cmp_key : CmpFn<Nat>, cmp_val : CmpFn<Nat>, leaf_node : Leaf<Nat, Nat>, elem_index : Nat, key : Nat, val : Nat) : ?Nat {
        // Debug.print("replace");

        let ?prev = leaf_node.3 [elem_index] else Debug.trap("1. insert: accessed a null value while replacing a key-value pair");
        let ?_prev_max = MaxHeap.peekMax(leaf_node.5) else Debug.trap("1: insert: should have a max value");
        var prev_max = _prev_max;

        let elem = (key, val);
        Leaf.replace(leaf_node, cmp_key, cmp_val, elem_index, elem);
        ...
        if (cmp_key(key, prev.0) != 0) {
            ...
            switch (leaf_node.1 [C.PARENT]) {
                case (?parent) if (elem_index == 0) {
                    Branch.update_median_key<Nat, Nat>(parent, leaf_node.0 [C.INDEX], key);
                };
                case (null) {};
            };
        };
        ...
        let ?_new_max = MaxHeap.peekMax(leaf_node.5) else Debug.trap("1: insert (replace entry): should have a max value");
        var new_max = _new_max;

        func update_path_upstream(branch : Branch<Nat, Nat>, child_index : Nat) {
            // need not increase the subtree size because it's replacing an existing key-value pair
            ...
            if (Utils.tuple_cmp(cmp_key, cmp_val)(prev_max, new_max) == 0) return;

            let ?old_branch_max = MaxHeap.peekMax(branch.5) else Debug.trap("replace(update_path_upstream): should have a max value");
            MaxHeap.replace(branch.5, Utils.tuple_cmp(cmp_key, cmp_val), branch.0 [C.COUNT], prev_max, new_max);

            let ?new_branch_max = MaxHeap.peekMax(branch.5) else Debug.trap("replace(update_path_upstream): should have a max value");

            prev_max := old_branch_max;
            new_max := new_branch_max;
        };
    ...
    public func _insert_at_leaf_index(max_bp_tree : MaxBpTree<Nat, Nat>, cmp_key : CmpFn<Nat>, cmp_val : CmpFn<Nat>, leaf_node : Leaf<Nat, Nat>, elem_index : Nat, key : Nat, val : Nat) {

        let entry = (key, val);

        if (leaf_node.0 [C.COUNT] < max_bp_tree.order) {

            let _prev_max = MaxHeap.peekMax(leaf_node.5);
            // Debug.print("heap before insert: " # debug_show leaf_node.5);

            Leaf.insert(leaf_node, cmp_key, cmp_val, elem_index, entry);
            max_bp_tree.size += 1;

            let ?max = MaxHeap.peekMax(leaf_node.5) else Debug.trap("1: insert: should have a max value");
            var prev_max = switch (_prev_max) {
                case (?max) max;
                case (_) return; // only occurs if the entry is the first entry in the btree
            };

            var new_max = max;

            // Debug.print("space in leaf");
            // Debug.print("heap after insert: " # debug_show leaf_node.5);
            ...
            func update_path_upstream(branch : Branch<Nat, Nat>, child_index : Nat) {
                ...
                // Debug.print("leaf_to_root maxheap repalce");
                // Debug.print("prev max: " # debug_show prev_max);
                // Debug.print("new max: " # debug_show new_max);
                if (Utils.tuple_cmp(cmp_key, cmp_val)(prev_max, new_max) == 0) return;

                // Debug.print("branch heap before replace: " # debug_show branch.5);

                let ?branch_max = MaxHeap.peekMax(branch.5) else Debug.trap("insert: should have a max value");
                // Debug.print("replacing in leaf_to_root update");
                MaxHeap.replace(branch.5, Utils.tuple_cmp(cmp_key, cmp_val), branch.0 [C.COUNT], prev_max, new_max);

                // Debug.print("branch heap after replace: " # debug_show branch.5);

                let ?new_branch_max = MaxHeap.peekMax(branch.5) else Debug.trap("insert: should have a max value");
        ...
        switch (leaf_node.1 [C.PARENT]) {
            case (?grand_parent) {
                // Debug.print("parent before leaf split: " # debug_show grand_parent.5);
            };
            case (null) {};
        };

        // Debug.print("heap before leaf split: " # debug_show leaf_node.5);

        let ?_prev_left_max = MaxHeap.peekMax(leaf_node.5) else Debug.trap("1: insert: should have a max value");
        var prev_left_max = _prev_left_max;

        switch (leaf_node.1 [C.PARENT]) {
            case (?grand_parent) {
                // Debug.print("parent before leaf split: " # debug_show grand_parent.5);
                let ?parent_max = MaxHeap.peekMax(grand_parent.5) else Debug.trap("1: insert: should have a max value");
                prev_left_max := parent_max;
            };
            case (null) {};
        };

        let right_leaf_node = Leaf.split(leaf_node, elem_index, entry, gen_id, cmp_key, cmp_val, _prev_left_max);
        ...
        // Debug.print("left heap: " # debug_show leaf_node.5);
        // Debug.print("right heap: " # debug_show right_leaf_node.5);
        ...
        label index_split_loop while (Option.isSome(opt_parent)) {
            ...
            if (parent.0 [C.COUNT] < max_bp_tree.order) {
                ...
                // Debug.print("insert in branch node heap: " # debug_show parent.5);
                while (j >= right_index) {
                    if (j == right_index) {
                ...
                // assert Utils.validate_array_equal_count(parent.5, parent.0 [C.COUNT]);
                let ? #leaf(left_branch_node) or ? #branch(left_branch_node) = parent.3 [right_index - 1] else Debug.trap("3. insert: accessed a null value");
                let ?left_chiLd_max = MaxHeap.peekMax(left_branch_node.5) else Debug.trap("3. insert: should have a max value");

                let #leaf(right_branch_node) or #branch(right_branch_node) = right_node else Debug.trap("3. insert: accessed a null value");
                let ?right_child_max = MaxHeap.peekMax(right_branch_node.5) else Debug.trap("3. insert: should have a max value");

                let entry_is_left_max = cmp_key(left_chiLd_max.0, key) == 0;

                // Debug.print("left in insert: " # debug_show left_branch_node.5);
                // Debug.print("right in insert: " # debug_show right_branch_node.5);

                // Debug.print("right_child_max: " # debug_show right_child_max);
                MaxHeap.put(parent.5, Utils.tuple_cmp(cmp_key, cmp_val), right_child_max, parent.0 [C.COUNT]);
                // Debug.print("parent count : " # debug_show parent.0[C.COUNT]);
                // Debug.print("heap after insert: " # debug_show parent.5);

                parent.0 [C.COUNT] += 1;
                max_bp_tree.size += 1;

                // if (entry_is_left_max) {
                //     let ?a = left_node.5[1] else Debug.trap("3. insert: should have a max value");
                //     let ?b = left_node.5[2] else Debug.trap("3. insert: should have a max value");

                //     let second_max = if (Utils.tuple_cmp(cmp_key, cmp_val)(a, b) == 1) {a} else {b};

                // Debug.print("entry_is_left_max replace");
                //     MaxHeap.replace(parent.5, Utils.tuple_cmp(cmp_key, cmp_val), parent.0[C.COUNT], second_max, entry);
                // };

                let parent_prev_max = do {
                    let ?max = MaxHeap.peekMax(parent.5) else Debug.trap("3. insert: should have a max value");

                    if (Utils.tuple_cmp(cmp_key, cmp_val)(max, entry) != 0) {
                        max;
                    } else {
                        let ?a = parent.5 [1] else Debug.trap("3. insert: should have a max value");
                        let opt_b = parent.5 [2] else Debug.trap("3. insert: should have a max value");

                        if (Option.isNull(opt_b)) a else {
                            let ?b = opt_b else Debug.trap("3. insert: should have a max value");

                            if (Utils.tuple_cmp(cmp_key, cmp_val)(a, b) == 1) {
                                a;
                            } else {
                                b;
                            };
                ...
                // Debug.print("after insert in branch node heap: " # debug_show parent.5);

                var prev_max = prev_left_max;
                let ?new_branch_max = MaxHeap.peekMax(parent.5) else Debug.trap("3. insert: should have a max value");
                var new_max = new_branch_max;
                ...
                func update_path_upstream(branch : Branch<Nat, Nat>, child_index : Nat) {
                    ...
                    // Debug.print("branch_to_root maxheap repalce");
                    // Debug.print("prev max: " # debug_show prev_max);
                    // Debug.print("new max: " # debug_show new_max);
                    if (Utils.tuple_cmp(cmp_key, cmp_val)(prev_max, new_max) == 0) return;

                    let ?branch_max = MaxHeap.peekMax(branch.5) else Debug.trap("insert: should have a max value");
                    // Debug.print("before update_path replace: " # debug_show (branch.5));
                    MaxHeap.replace(branch.5, Utils.tuple_cmp(cmp_key, cmp_val), branch.0 [C.COUNT], prev_max, new_max);
                    // Debug.print("after  update_path replace: " # debug_show (branch.5));
                ...
                let _prev_left_max = prev_left_max;

                switch (parent.1 [C.PARENT]) {
                    case (?grand_parent) {
                        // Debug.print("parent before branch split: " # debug_show grand_parent.5);
                        let ?parent_max = MaxHeap.peekMax(grand_parent.5) else Debug.trap("1: insert: should have a max value");
                        prev_left_max := parent_max;
                    };
                    case (null) {};
                ...
                // Debug.print("branch heap before split: " # debug_show parent.5);

                // Debug.print("retrieved prev max: " # debug_show _prev_left_max);
                // prev_left_max := if (cmp_key(_prev_left_max.0, key) != 0) _prev_left_max else prev_left_max;
                // Debug.print("prev left max: " # debug_show prev_left_max);

                // Debug.print("before split branch node: " # debug_show (parent.4[C.MAX], Methods.children_max(parent)));
                let split_node = Branch.split(parent, right_node, right_index, right_key, gen_id, cmp_key, cmp_val, _prev_left_max);
                ...
                // Debug.print("left branch heap: " # debug_show parent.5);
                // Debug.print("right branch heap: " # debug_show split_node.5);
        ...
        // Debug.print("root heap: " # debug_show root_node.5);
    ...
    public func toLeafNodes<K, V>(self : MaxBpTree<K, V>) : [(?(K, V), [?(K, V)])] {
        Methods.to_leaf_nodes(self);
    };

    public func toNodeKeys<K, V>(self : MaxBpTree<K, V>) : [[(Nat, ?(K, V), [?K])]] {
    ...
    ///    let max_bp_tree = MaxBpTree.fromArray<Char, Nat>(arr, Cmp.Char, Cmp.Nat, null);
    /// ```
    public func fromArray(order : ?Nat, arr : [(Nat, Nat)], cmp_key : CmpFn<Nat>, cmp_val : CmpFn<Nat>) : MaxBpTree<Nat, Nat> {
        let max_bp_tree = MaxBpTree.new<Nat, Nat>(order);
    ...
    public func replaceMaxValue(self : MaxBpTree<Nat, Nat>, cmp_key : CmpFn<Nat>, cmp_val : CmpFn<Nat>, new_val : Nat) : ?(Nat, Nat) {
        if (self.size == 0) return null;
